...
void processJump(/*uint24_t*/ uint32_t  pos) {
...
void processRunJump(Prince &prince, Level &level, bool testEnemy) {
    ...
    if (jumpResult != RunningJumpResult::KeepRunning) {
        ...
        /*uint24_t*/ uint32_t  pos = RunningJumpStances + static_cast</*uint24_t*/ uint32_t >(static_cast<uint16_t>(jumpResult) * 16);
        processJump(pos);
...
void processStandingJump(Prince &prince, Level &level) {
    ...
    /*uint24_t*/ uint32_t  pos = StandingJumpStances + static_cast</*uint24_t*/ uint32_t >(static_cast<uint16_t>(standingJumpResult) * 16);
    processJump(pos);

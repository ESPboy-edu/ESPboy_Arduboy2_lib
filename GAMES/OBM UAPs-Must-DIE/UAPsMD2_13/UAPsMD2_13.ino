...
// UAP bullets
int16_t UAPbulletx[4];
int16_t UAPbullety[4];
...
// gennerate initial grid
  memset (ter, sizeof(ter), 0);

// generate stars
for (int16_t i = 1; i < 75; i++){
  starx[i] = rand() % 127;
  stary[i] = rand() % 63;
  staroe[i] = rand() % 2; // create 2 different kind of stars at two different speeds
 }
}
...
void run_menu(){
  ...
  while(1){
    ...
    if (arduboy.pressed(UP_BUTTON)) s = 1;
    if (arduboy.pressed(DOWN_BUTTON)) s = 0;
    if (arduboy.pressed(LEFT_BUTTON)) FPS = 25;
    if (arduboy.pressed(RIGHT_BUTTON)) FPS = 50;
  ...
  else
{
  if (UAPexist[1] == 1)
  {
    ...
    int16_t r = rand() % 300; // Randomly initiate Enemy bullets
    {
      if (r < 5)
      {
...
planetx[1] = 60;
planety[1] = 170;
for (int16_t i = 1; i < 75; i++) // resposition stars for win screen
{
  int16_t rx = rand() % 127;
  starx[i] = rx;
  int16_t ry = rand() % 63;
...
for (int16_t c = 1; c < 180; c++) // Win screen loop
{
  arduboy.clear();
  for (int16_t i = 1; i < 75; i++) // Move and Draw Stars for win screen
  {
    if (staroe[i] == 0)
    {
    stary[i] = stary[i] - 1;
...
arduboy.display();
arduboy.delayShort(500);
for (int16_t i = 1; i < 75; i++) // resposition stars for die screen
{
  int16_t rx = rand() % 127;
  starx[i] = rx;
  int16_t ry = rand() % 63;
...
for (int16_t c = 1; c < 180; c++) // Die screen loop
{
  arduboy.clear();
  for (int16_t i = 1; i < 75; i++) // Move and Draw Stars for Die screen
  {
    if (staroe[i] == 0)
    {
    starx[i] = starx[i] + 1;
...
for (int16_t i = 1 ; i < 4; i++) // deactivate out of bounds debris
{
  if (debrisx[i] > 160)
  {
    debrisExist[i] = 0;
...
void ResetLevel()
{
for (int16_t x = 1; x < 16 ; x++) // Wipe Terrain for new game
{
...
// Move Player
     ...
     if (arduboy.pressed(UP_BUTTON) && PSY>30){
       PSX=PSX-4;
       PSY=PSY-2;
       Left = 1;
     }

     if (arduboy.pressed(RIGHT_BUTTON) && PSY<51){
       PSX=PSX+4;
       PSY=PSY+2;
       Right = 1;
     }

     if (arduboy.pressed(LEFT_BUTTON) && Left == 0 && PSY>30)  {
       PSX=PSX-4;
       PSY=PSY-2;
       Left = 1;
     }

    if (arduboy.pressed(DOWN_BUTTON) && Right == 0 && PSY<51){
        PSX=PSX+4;
        PSY=PSY+2;
        Right = 1;
    } 
...
if (planetx[2] < -120) planetExist[2] =0;
...
// Draw Terrain
for (int16_t xs = 126; xs > -20; xs=xs-10)
{
  scanx++;
  
  for (int16_t rowx=0; rowx < 50; rowx=rowx+10)
  {
    z = ter[scanx][scany];
    ...
    int16_t xoffset = xs+rowx-scrollScreenX-10;
    int16_t yoffset = ys+rowy+scrollScreenY-5;
...
for (int16_t i = 0 ; i< 4 ; i ++) // Move UAPbullet and detect Collision with Ship
{
...
int16_t SCREEN = 9999 - LevelProgress;
...
int16_t Rocky = rand() % 100; // Create varied "Rockyness"
...
// Spawn UAPs *****************************
int16_t r = rand() % 3000; // Rate at which UAPs Spawn
if (r < 100)
  spawnUAP();
...
for (int16_t x = 1; x < 16 ; x++)
{
  ...
  for(int16_t y = 1; y < 8 ; y++)
  {
    int16_t r = rand() % height; // Blocks up to 14 ?? +1
    ter[1][y] = r;
  }
}
  LevelProgress++;

...
void drawLogo(void)
{
    ...
    //ab.setRGBled(0, 0, isSignalOn * 127);
    ab.clear();
    int shake = (COUNTER_MAX - counter) / (44 * FPS / 60);
    for (int i = 0; i < 3; i++) {
        int y = 12 + (i == shake) * isSignalOn;
        ab.drawBitmap(28 + i * 24, y, imgOBN[i], 24, 32, WHITE);
    }
    ab.drawBitmap(68, 44, imgSoft, 32, 8, WHITE);
    ab.printEx(16, 58, F(APP_CODE " VER " APP_VERSION));
    ab.printEx(7, 0, F("#ARDUBOY BANANA JAM"));
}
